 ...
 *
 * @package   Astha
 * @author    Astha Dev Team
 * @copyright Copyright (c) 2015 - 2021, Astha Dev Team
 ...
 *
 * @package Astha\Core\Models\AB_Model
 * @author  Astha Dev Team
...
class AB_Model extends CI_Model
{
    ...
    public function find_by($field = '', $value = '', $type = 'and')
    {
        if (empty($field) || ( ! is_array($field) && empty($value))) {
            $this->error = lang('AB_Model_find_error');
            $this->logit('[' . get_class($this) . ': ' . __METHOD__ . '] ' . lang('AB_Model_find_error'));
            return false;
        }
    ...
    public function update($where = null, $data = null)
    {
        ...
        $this->error = sprintf(lang('AB_Model_db_error'), $this->get_db_error_message());
        return false;
    }
    ...
    public function is_unique($field = '', $value = '')
    {
        if (empty($field) || empty($value)) {
            $this->error = lang('AB_Model_unique_error');
            $this->logit('[' . get_class($this) . ': ' . __METHOD__ . '] ' . lang('AB_Model_unique_error'));
            return false;
        }
    ...
    {
        if (empty($field)) {
            $this->error = lang('AB_Model_count_error');
            $this->logit('[' . get_class($this) . ': ' . __METHOD__ . '] ' . lang('AB_Model_count_error'));
            return false;
        }
    ...
    public function get_field($id = null, $field = '')
    {
        if (empty($id) || empty($field)) {
            $this->error = lang('AB_Model_fetch_error');
            $this->logit('[' . get_class($this) . ': ' . __METHOD__ . '] ' . lang('AB_Model_fetch_error'));
            return false;
        }
     ...
     *
     * @return AB_Model An instance of this class.
     */
     ...
     *
     * @return AB_Model An instance of this class to allow method chaining.
     */
